...
app.get('/stats', async (c) => {
  ...
  const { data, error } = await supabase
    .from('sensor_readings')
    .select('temperature, turbidity_ntu, moisture_pct')
  ...
  const calc = (arr, key) => {
    const vals = arr.map(r => r[key]).filter(v => v !== null && v !== -999);
  ...
  return c.json({
    ...
    stats: {
      ...
      turbidity_ntu: calc(data, 'turbidity_ntu'),
      moisture_pct:  calc(data, 'moisture_pct'),
    },
  });
});
